// Minimal EVM test harness for hyperveil/evm: compiles contracts/ + test/Mocks.sol
// and runs them in @ethereumjs/evm. No node, no chain -- real bytecode executing
// real opcodes. The same harness as the Veil bridge's (bridge/evm/test), plus
// `deployAt`, which places a contract's runtime code at a fixed address: that
// is how HyperEVM's precompiles and CoreWriter are stood in for.
const fs = require('fs');
...
class Chain_ {
  constructor(evm) {
    ...
    this.blockNumber = 0;
    this.logs = [];
    // Block timestamp for every call. Unset, calls run in @ethereumjs's
    // default block (timestamp 0), as every suite written before this did.
    this.time = undefined;
  }

  /// Run every later call at block timestamp `ts` (seconds).
  setTime(ts) {
    this.time = BigInt(ts);
  }

  /// The block a call runs in, or undefined for the EVM's default.
  blockArg() {
    if (this.time === undefined) return undefined;
    return {
      header: {
        number: BigInt(this.blockNumber + 1),
  ...
  /// Deploys `name` normally, then copies its runtime code to `address` (a
  /// precompile or system-contract address). Storage at `address` starts empty.
  async deployAt(name, address, from = 1) {
    const c = await this.deploy(name, [], from);
    const code = await this.evm.stateManager.getContractCode(c.address);
    const target = new Address(hexToBytes(address));
    await this.evm.stateManager.putContractCode(target, code);
    return new Contract(this, name, target, c.iface);
  }

  async balanceOf(address) {
    const acct = await this.evm.stateManager.getAccount(new Address(hexToBytes(address)));
...
class Contract {
  ...
  async call(fn, args = [], from = 1, value = 0n) {
    ...
    const res = await this.chain.evm.runCall({
      ...
      gasLimit: 100000000n,
      value,
      // A call carrying value funds its caller first (native HYPE in tests).
      skipBalance: value > 0n,
      block: this.chain.blockArg(),
    });
